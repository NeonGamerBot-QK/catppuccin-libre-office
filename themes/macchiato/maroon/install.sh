#!/bin/sh
echo "Installation for macchiato - maroon"
...
existing_theme="$(grep -n \
  '<item oor:path="/org.openoffice.Office.UI/ColorScheme/ColorSchemes"><node oor:name="Catppuccin macchiato - maroon"' \
  "$fname")"
exit_code=$?
...
if [ $exit_code -eq 0 ]; then
  ...
  new_settings="$(echo "$settings_start" \
    && cat ./catppuccin-macchiato-maroon.xcu \
    && echo "$settings_end")"
else
  # Insert theme between last two lines if not present
  new_settings="$(head -n $(($(wc < "$fname" -l) - 1)) "$fname" && cat ./catppuccin-macchiato-maroon.xcu && tail -n1 "$fname")"
fi
