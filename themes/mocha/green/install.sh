#!/bin/sh
echo "Installation for mocha - green"
...
existing_theme="$(grep -n \
	'<item oor:path="/org.openoffice.Office.UI/ColorScheme/ColorSchemes"><node oor:name="Catppuccin mocha - green"' \
	"$fname")"
exit_code=$?
...
if [ $exit_code -eq 0 ]; then
	...
	new_settings="$(echo "$settings_start" \
	             && cat ./catppuccin-mocha-green.xcu \
	             && echo "$settings_end")"
else
	# Insert theme between last two lines if not present
	new_settings="$(head -n $(($(<"$fname" wc -l)-1)) "$fname" && cat ./catppuccin-mocha-green.xcu && tail -n1 "$fname")"
fi
