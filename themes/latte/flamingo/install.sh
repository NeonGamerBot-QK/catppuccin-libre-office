#!/bin/sh
echo "Installation for latte - flamingo"
...
existing_theme="$(grep -n \
	'<item oor:path="/org.openoffice.Office.UI/ColorScheme/ColorSchemes"><node oor:name="Catppuccin latte - flamingo"' \
	"$fname")"
exit_code=$?
...
if [ $exit_code -eq 0 ]; then
	...
	new_settings="$(echo "$settings_start" \
	             && cat ./catppuccin-latte-flamingo.xcu \
	             && echo "$settings_end")"
else
	# Insert theme between last two lines if not present
	new_settings="$(head -n $(($(<"$fname" wc -l)-1)) "$fname" && cat ./catppuccin-latte-flamingo.xcu && tail -n1 "$fname")"
fi
